...
#Dehydroascorbate redutase (DHAR) - Antioxidant enzyme - GWAS with PC 1-5 as covariate
```{r}
assocRes_MLM_DHAR <- assocModelFitter(geno_pheno_filtered, formula = DHAR ~ Treatment + PC1 + PC2 + PC3, fitMarkers = TRUE, kinship = kin, maxP = 1)
```

#DHAR plotting - PC 1-5
```{r fig.height=4, fig.width=12}
plotManhattan(assocRes_MLM_DHAR, trait = "DHAR", threshold = 4, interactive = FALSE, colors = c("#00AFBB", "#FC4E07"))
```
...
```{r}
assocRes_MLM_TAC <- assocModelFitter(geno_pheno_filtered, formula = TAC ~ Treatment + PC1 + PC2 + PC3, fitMarkers = TRUE, kinship = kin, maxP = 1)
```
